...
SkipEmulatedInstruction(void)
{
    ...
    __asm_vmx_vmwrite(0x0000681E, RIP + Length);
}
...
{
    size_t ExitInterruptionInfo = {};
    size_t ExitInterruptionErrorCode = {};
    size_t ExitInstructionLength = {};

    __asm_vmx_vmread(VMX_VMCS32_RO_EXIT_INTERRUPTION_INFO, &ExitInterruptionInfo);

    uint8_t Vector = (uint8_t)((ExitInterruptionInfo >> 0x00) & 0xFF);         // Bits 7:0
    uint8_t Type = (uint8_t)((ExitInterruptionInfo >> 0x08) & 0x07);           // Bits 10:8
    uint8_t ErrorCodeValid = (uint8_t)((ExitInterruptionInfo >> 0x0B) & 0x01); // Bit 11
    uint8_t Valid = (uint8_t)((ExitInterruptionInfo >> 0x1F) & 0x01);          // Bit 31

    if (!Valid)
        return;

    if (ErrorCodeValid)
        __asm_vmx_vmread(VMX_VMCS32_RO_EXIT_INTERRUPTION_ERROR_CODE, &ExitInterruptionErrorCode);

    // 先清掉上一次可能残留的注入辅助字段，再重新写本次事件。
    __asm_vmx_vmwrite(VMX_VMCS32_CTRL_ENTRY_INTERRUPTION_INFO, 0);
    __asm_vmx_vmwrite(VMX_VMCS32_CTRL_ENTRY_EXCEPTION_ERRCODE, 0);
    __asm_vmx_vmwrite(VMX_VMCS32_CTRL_ENTRY_INSTR_LENGTH, 0);

    BuildEvent(Vector, Type, (uint32_t)ExitInterruptionErrorCode);

    // 软件类事件（INT n / ICEBP / INT3 / INTO）在 VM-entry 时还需要 instruction length。
    switch (Type) {
    case 0x4: // Software interrupt
    case 0x5: // Privileged software exception
    case 0x6: // Software exception
        __asm_vmx_vmread(VMX_VMCS32_RO_EXIT_INSTR_LENGTH, &ExitInstructionLength);
        __asm_vmx_vmwrite(VMX_VMCS32_CTRL_ENTRY_INSTR_LENGTH, ExitInstructionLength);
        break;
    default:
        break;
    }
...
procedure<0x000A>(VMCpu*, VMContext* ctx)
{
    ...
    case 0x88888888: {
        ...
        *(uint8_t*)(p + 0x1003) = 0xC3; // RET
        ...
        // 不调用 RebuildEPT, 不设 TrapPage. p+0x1000 那页走初始化时已经建好的普通 EPT 映射.
...
template <>
void
procedure<0x001F>(VMCpu*, VMContext* ctx)
{
...
template <>
void
procedure<0x0020>(VMCpu*, VMContext* ctx)
{
...
procedure<0x0025>(VMCpu* vcpu, VMContext*)
{
    ...
    g_SingleStepCount++;
    if (g_SingleStepCount <= 3)
        KdPrint(("procedure<0x0025>: RIP=0x%016llX [StpCnt=%llu]\n", RIP, g_SingleStepCount));
...
procedure<0x0030>(VMCpu* vcpu, VMContext*)
{
    ...
    if (ViolationX) {
        ...
        KdPrint(("procedure<0x0030>: RIP=0x%016llX, LA=0x%016llX, PA=0x%016llX [VioCnt=%llu]\n", RIP, GuestLinearAddress, GuestPhysicalAddress, g_ViolationXCount));
        KdPrint(("vcpu->Tmp = %d\n", vcpu->Tmp));
    } else {
...
HandleVmExit2(VMContext* ctx)
{
    ...
    size_t ExitReason = {};
    __asm_vmx_vmread(0x00004402, &ExitReason);
    ...
    __asm_vmx_vmread(0x00000800, &es); // Guest ES selector
    __asm_vmx_vmread(0x00000802, &cs); // Guest CS selector
    __asm_vmx_vmread(0x00000804, &ss); // Guest SS selector
    __asm_vmx_vmread(0x00000806, &ds); // Guest DS selector
    __asm_vmx_vmread(0x00000808, &fs); // Guest FS selector
    __asm_vmx_vmread(0x0000080A, &gs); // Guest GS selector
    ...
    KmEvent->EventType = ExitReason & 0xFFFF;

    __asm_vmx_vmread(VMX_VMCS_GUEST_CR3, &KmEvent->CR3);
    __asm_vmx_vmread(VMX_VMCS_GUEST_RIP, &KmEvent->RIP);
    ...
    Procedures[ExitReason & 0xFFFF](vcpu, ctx); // Bits 15:0 of this field contain the basic exit reason.
}
...
#define EFLAGS_ID_MASK   (1 << 0x15)
    ...
    vcpu->VMCSGuest = vmcsGuest;
    vcpu->VMCSGuestPa = vmcsGuestPA;
    vcpu->VMXONRegion = vmxonRegion;
    vcpu->VMXONRegionPa = vmxonRegionPA;
    ...
    // Host Fields
    {
        Status |= __asm_vmx_vmwrite(VMX_VMCS_HOST_RSP, (size_t)vcpu + KM_FRAME_SIZE);
